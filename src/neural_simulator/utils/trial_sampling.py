...
class Uniform(Distribution):
    ...
    def sample(self, n: int = 1):
        return self.rng.uniform(low=self.low, high=self.high, size=n)
...
class LogUniform(Distribution):
    ...
    def sample(self, n: int = 1):
        return np.exp(self.rng.uniform(low=np.log(self.low), high=np.log(self.high), size=n))
...
class Normal(Distribution):
    ...
    def sample(self, n: int = 1):
        return self.rng.normal(loc=self.mean, scale=self.std, size=n)
...
class SampleSpace:
    def __init__(
        ...
        self.seed(seed)
        self._distributions = {
            key: val if isinstance(val, Distribution) else self._build_distribution(**val)
    ...
    def sample(self, n: int = 1, stratified: bool = False):
        ...
        if stratified: # only do stratified sampling over categorical fields
            categorical_keys = [key for key, val in self._distributions.items() if isinstance(val, Categorical)]
            all_categories = [self._distributions[key].categories for key in categorical_keys]
            ...
            stratified_sample = np.concatenate([
                ...
                all_combinations[self.rng.choice(a=len(all_combinations), size=(n % len(all_combinations)))]
            ], axis=0)
