...
class SklearnEmbedding(Embedding):
    def __init__(self, estimator: BaseEstimator, seed: Optional[Union[int, np.random.Generator]] = None):
    ...
    def seed(self, seed: Optional[Union[int, np.random.Generator]] = None):
        super().seed(seed)
        if 'random_state' in inspect.signature(self.estimator.__init__).parameters.keys():
...
class EmbeddingStack(Embedding):
    def __init__(self, embeddings: list[Embedding], seed: Optional[Union[int, np.random.Generator]] = None):
